...
# E.G. str class has a lot of functions defined for the object which we can access just by pressing tab. Could write functions for a string object, 
# but would not be as easily accessible by simply pressing the tab button
# E.G. 2 RandomForestClassifier() bundles all the classifier methods together too e.g. fit/predict

# 2. Can help make code more modular 

# 3. Allows us to group data into an object in a way that is easier to read and understand than organising data into lists and dictionaries

# A class has a __init__ method, __call__ method, and any other custom methods you create


class My_Class: #Definition of My_class
    x = None  # Means every object instantiated will have an x parameter with value equal to 5

instance_a = My_Class()
instance_a.x = 5 # Parameter x of instance_a now has a different value
...
class Dog:
    #default constructor
    # Use the init method when you want to assign values to the parameters of your class when your instance is created
    def __init__(self, breed):
        self.tail = 'Yes'    # default value
        self.breed = breed
        self.value = 1000
labrador_instance = Dog('labrador')
labrador_instance.breed # returns the parameter value for that instance

    def __call__(self, popularity_multiplier): 
        self.value *= popularity_multiplier   #
        print(self.value)


#### Can have a subclass of a class - will inherit init and call method and any other custom methods. Can rewrite methods within subclass if want to overwrite. i.e. can overwrite inherited methods
